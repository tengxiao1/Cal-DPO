...
from accelerate import PartialState # diff -
from accelerate.utils import is_deepspeed_available, tqdm
from datasets import Dataset
from huggingface_hub.utils._deprecation import _deprecate_arguments # diff
...
from trl.trainer.utils import (
    DPODataCollatorWithPadding,  # diff
    RunningMoments,  # diff
    disable_dropout_in_model,
    pad_to_length,
    peft_module_casting_to_bf16,  # diff
    trl_sanitze_kwargs_for_tagging,  # diff -
)
...
class DPOTrainer(Trainer):
    ...
    def __init__(
            ...
            args: Optional[DPOConfig] = None,# diff
            data_collator: Optional[DataCollator] = None,
            label_pad_token_id: int = -100,
            padding_value: Optional[int] = None, # diff
            ...
            precompute_ref_log_probs: bool = False,# diff
            dataset_num_proc: Optional[int] = None,
            model_init_kwargs: Optional[Dict] = None,
            ref_model_init_kwargs: Optional[Dict] = None,
            model_adapter_name: Optional[str] = None,
            ref_adapter_name: Optional[str] = None,
            reference_free: bool = False,# diff
            force_use_ref_model: bool = False,# diff
    ):
        ...
        with PartialState().local_main_process_first():
            ...
            train_dataset = train_dataset.map(self.tokenize_row_mingxiao, num_proc=self.dataset_num_proc)
            if eval_dataset is not None:
                eval_dataset = eval_dataset.map(self.tokenize_row_mingxiao, num_proc=self.dataset_num_proc)
    ...
    def build_tokenized_answer(self, prompt, answer): # diff mingxiaoli
        """
        ...
        # 获得 answer 的 input_ids 和 attention mask
    ...
    def tokenize_row_mingxiao(self, feature, model: Optional[Union[PreTrainedModel, nn.Module]] = None) -> Dict:
        """Tokenize a single row from a DPO specific dataset.
        ...
        if not self.is_encoder_decoder:
            ...
            if len(prompt_tokens["input_ids"]) + longer_response_length > self.max_length:
                chosen_tokens = {
                    k: v[: self.max_length - self.max_prompt_length] for k, v in chosen_tokens.items()
                }
                rejected_tokens = {
    ...
    def concatenated_inputs(
        ...
        if is_encoder_decoder:
            concatenated_batch["concatenated_input_ids"] = batch["prompt_input_ids"].repeat(2, 1).to(device=device) #diff
            concatenated_batch["concatenated_attention_mask"] = (
                batch["prompt_attention_mask"].repeat(2, 1).to(device=device) #diff
            )
    ...
    def concatenated_forward(
        ...
        concatenated_batch = self.concatenated_inputs(
            batch,
            is_encoder_decoder=self.is_encoder_decoder, # diff
            label_pad_token_id=self.label_pad_token_id,# diff
            padding_value=self.padding_value,# diff
            device=self.accelerator.device,# diff
        )
        ...
        all_logits = model(
            ...
            use_cache=False,
            **model_kwargs,
        ).logits.to(torch.float32)  # mingxiaoli
        # all_logits = model(
        #     concatenated_batch["concatenated_input_ids"],
        #     attention_mask=concatenated_batch["concatenated_attention_mask"],
        #     use_cache=False,
        ...
        all_logps = self.get_batch_logps(
            ...
            average_log_prob=self.loss_type == "Cal-DPO-average",
            is_encoder_decoder=self.is_encoder_decoder,
